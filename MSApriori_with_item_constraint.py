...
def find_cannot_be_together():
	winCmd = "java -jar spmf.jar run FPGrowth_itemsets retail1.txt fpgrowth_out.txt 2%"
	check_output(winCmd)
	fpgrowth_out = open('fpgrowth_out.txt', "r")
	fpgrowth_out = fpgrowth_out.readlines()
	fpgrowth_res = {}
	for f in fpgrowth_out:
		f_split = (f.rstrip()).split(" #SUP: ")
		if len(f.split()) > 3:
			fpgrowth_res[f_split[0]] = int(f_split[1])
	fpgrowth_res = sorted(fpgrowth_res,key=fpgrowth_res.get,reverse=True)[0:5]
	return fpgrowth_res

def find_must_have_set(ipset):
	opset = set([iset.split()[0] for iset in ipset])
	return opset
	
def check_item_const(ip_itemset):
	item_const_passed = True
	for sub in cannot_be_together_set :
		sub=sub.split(" ")
		sub=list(map(int,sub))
		if set(sub) <= set(ip_itemset):
			item_const_passed = False
	if item_const_passed == True:
		must_have_set_chk = set(must_have_set).intersection(set(ip_itemset))
		if len(must_have_set_chk) == 0:
			item_const_passed = False
	return item_const_passed
...
## Create cannot_be_together and must_have_sets
cannot_be_together_set = find_cannot_be_together()
must_have_set = find_must_have_set(cannot_be_together_set)
must_have_set = set(map(int,must_have_set))
...
while (end_of_loop == 0):
	...
	if len(fk) > 0: 
		...
		print("Total time taken for MSApriori algorithm (with item constraint): " + str(elapsed_time))
		print(" ")
		print("End of code!")
		end_of_loop = 1